...
public class MyPageController {
    ...
    @ApiOperation(value = "마이페이지 클릭 시 내 주식 상세 정보 포함하여 반환")
    @GetMapping("/stock")
    ...
    @ApiOperation(value = "마이페이지에서 예금 버튼 클릭 시 반환")
    @GetMapping("/deposit")
    public ResponseEntity<?> myPageDepositDetail(@RequestHeader("Authorization") String tokens) {
        try {

            System.out.println("============= 마이페이지 예금 클릭 화면 =============");
            // 유저가 가지고 있는 주식에 대한 데이터 리스트
            List<MyStockDto> myStockDtoList = null;
            // 유저가 가지고 있는 예금에 대한 데이터
            MyDepositDto myDepositDto = null;
            ...
            // 유저의 포인트 정보와 예금 정보를 모아 리턴한다.
            MyPagePointAndDepositDto myPagePointAndDepositDto = new MyPagePointAndDepositDto(myPointDto, myDepositDto);
            return new ResponseEntity<>(myPagePointAndDepositDto,HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
    }
