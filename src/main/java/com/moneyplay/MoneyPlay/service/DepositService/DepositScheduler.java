    ...
    public void Scheduler() {
        ...
        for(int i=0; i<deposits.size(); i++){
            ...
            if(endDay == todayDay){
                ...
                if(todayYear == endYear && todayMonth == endMonth){


                    // 보유 금액 늘려주기 + 추가 이자 지급

                    user.getPoint().setHoldingPoint(user.getPoint().getHoldingPoint()+deposits.get(i).getInterestAmount() + deposits.get(i).getDepositAmount()* deposits.get(i).getDepositType().getDepositInterestRate());
                ...
                // 만기일은 아닐 때 : 시작 날짜랑 오늘이 같지 않다면-> 이자 올려주기
                    ...
                    if(startYear == todayYear && startMonth==todayMonth && startDay==todayDay){}
                        ...
                        deposits.get(i).setInterestAmount(deposits.get(i).getInterestAmount() + deposits.get(i).getDepositAmount()*deposits.get(i).getDepositType().getDepositInterestRate());
                    }
                }
